...
//   A. the PRODUCTION path: preprocess → active provider (fp16 on WebGPU, or
//      fp32 on WASM when WebGPU is unavailable — see config.ts)
//   B. the GROUND-TRUTH path: same preprocess → palm_256_l2_fp32.onnx (reference)
// and logs cosine(A, B). Expect ~0.99+ (fp16 quantization noise only when the
// WebGPU build is active; ~1.0 when the fp32 build is active). A large gap
// means the preprocessing is wrong — most likely: forgot ÷255, wrong mean/std
// order, BGR vs RGB, or CLAHE accidentally applied at inference.
//
...
export async function runParitySelfTest(): Promise<ParityResult> {
  ...
  if (result.passed === true) {
    console.info(
      `[ML parity self-test] PASS — cosine(${result.provider}, fp32 reference) = ` +
        `${result.referenceCosine!.toFixed(6)} ≥ ${PARITY_MIN_COSINE}. Preprocessing matches training.`,
    );
  } else if (result.passed === false) {
    const c = result.referenceCosine!;
    console.error(
      `[ML parity self-test] FAIL — cosine(${result.provider}, fp32 reference) = ${c.toFixed(6)} ` +
        `< ${PARITY_MIN_COSINE}. Both active variants (fp16-web, fp32-web) should score ≥ 0.999 ` +
        `against the fp32 reference (see deploy_config.json quantization_drift). Check (most likely ` +
        `first): ÷255 scaling, mean/std values and order, RGB vs BGR, CLAHE/illumination ops ` +
        `accidentally applied at inference, or wrong .onnx files deployed.`,
    );
  }
  return result;
}
 ...
 * Same-palm pairs should score ≥ VERIFICATION_THRESHOLD (0.5346, FAR 0.1%);
