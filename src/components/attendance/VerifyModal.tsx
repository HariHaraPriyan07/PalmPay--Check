...
export function VerifyModal({
  ...
  const handleCapture = useCallback(
    async (capture: CaptureResult) => {
      ...
      try {
        ...
        // Decision (§4): accept ≥ 0.5216 (calibrated at FAR 0.1%), retry band
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Verify palm — ${student.name}`} wide>
      ...
      {step === "processing" && (
        <div className="flex items-center gap-3 py-8">
          <Spinner /> <span className="text-sm text-body">Matching…</span>
        </div>
      )}
          ...
          {error ? (
            ...
            <>
              ...
              {result.outcome === "retry" && (
                <div className="flex items-start gap-3 rounded-card bg-status-warn-bg p-4 text-status-warn-fg">
              ...
              {result.outcome === "reject" && (
                <div className="flex items-start gap-3 rounded-card bg-status-absent-bg p-4 text-status-absent-fg">
