...
//   • DEFAULT:  palm_int8_web.onnx on the WASM EP (fp32 I/O) — every laptop.
//   • OPT-IN:   palm_fp16_web.onnx on the WebGPU EP (fp16 I/O) — only when
//               ENABLE_WEBGPU is set AND the browser exposes WebGPU.
// Pairing is fixed (INT8↔WASM, FP16↔WebGPU — never crossed: the ConvInteger
// INT8 build does not accelerate on WebGPU). The fp32 reference build is used
// exclusively by the parity self-test (parity.ts).
//
...
async function runSession(
  ...
  // float16 for palm_fp16_web, float32 for the int8-web and fp32 models.
  const tensor =
  ...
  async init(): Promise<void> {
    ...
    if (wantWebgpu) {
      try {
        ...
        console.warn(
          "[ML] WebGPU session failed — falling back to INT8 on WASM (the safe default).",
          err,
        );
      }
    }
 ...
 * and the INT8 web build match training. Never used for real matching.
 */
